...
object MyModule {
  ...
  def fib(n: Int): Int = {
    require(n >= 1, "Fibbonacci elements starts from 1st element")

    @tailrec
    def innerFib(beforePrev: Int, prev: Int, currentN: Int): Int = {
      if(currentN == n) {
        prev
      } else {
        innerFib(prev, beforePrev + prev, currentN + 1)
      }
    }
...
object PolymorphicFunctions {
  ...
  def isSorted[A](as: Array[A], gt: (A,A) => Boolean): Boolean = {
    1.until(as.length).foreach { index =>
